...
// renderização.
float g_CameraTheta = -2.81f; // Ângulo no plano ZX em relação ao eixo Z
float g_CameraPhi = -0.26f;   // Ângulo em relação ao eixo Y
float g_CameraDistance = 2.5f; // Distância da câmera para a origem

// Variáveis da câmera Free Camera
glm::vec4 camera_position_c = glm::vec4(2,2,7,1.0f);
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 325315 - Pedro Oliveira Mews", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
